$(function() {
	...
	$("#submit").hover(
		function() {
			$("#submit").attr("src", getContextPath() + "/images/login2.jpg");
		},
		function() {
			$("#submit").attr("src", getContextPath() + "/images/login1.jpg");
		}
	);
...
function validateLoginname() {
	...
	if(!value) {// 非空校验
		...
		$("#loginnameError").css("display", "");
		$("#loginnameError").text("用户名长度必须在5 ~ 20之间！");
		bool = false;
	}
	return bool;
}
...
function validateLoginpass() {
	...
	if(!value) {// 非空校验
		...
		bool = false;
	} else if(value.length < 5 || value.length > 20) {//长度校验
		$("#loginpassError").css("display", "");
		$("#loginpassError").text("密码长度必须在5 ~ 20之间！");
		bool = false;
	}
	return bool;
}
